...
@Module({ dynamic: true, store, namespaced: true, name: "reactiveLocalStorage" })
class ReactiveLocalStorage extends VuexModule {
  localStore: Store = {
    showSettingsInNavigation: localStorage["showSettingsInNavigation"] === "true"
  };
  ...
  @Mutation
  private SET_STORE<K extends keyof Store>({key, value}: {key: K  , value: Store[K]}) {
