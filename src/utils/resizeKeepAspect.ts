...
export default function resizeKeepAspect(resizeEl: any, parentEl: any, width: number, height: number) {
	if (!parentEl) return {};
	...
	const newDimentions = calculateAspectRatioFit(
		srcWidth,
		srcHeight,
		clamp(minWidth, 200, 500),
		minHeight
	);

	resizeEl.style.height =
		clamp(newDimentions.height, 0, srcHeight) + "px";
